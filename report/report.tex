...
Figure~\ref{fig:survival-tau} shows the same survival estimates against $\tau=\beta\,\mathbb{E}[W]$ with $\tau_{\mathrm{pred}}=1/\lambda_{\max}$ (\textbf{RQ1}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{fig_er_survival_vs_tau.png}
  \caption{ER seed \RepPrimarySeed: fraction surviving vs.\ $\tau=\beta\,\mathbb{E}[W]$ ($\mathbb{E}[W]{=}5$ ticks). Dashed vertical line: $\tau_{\mathrm{pred}}$; dotted horizontal: 50\% survival.}
  \label{fig:survival-tau}
\end{figure}

Figure~\ref{fig:late-prev} complements the survival curves with \texttt{bs-out-late-mean-prevalence} on runs that are not extinct at the horizon (proposal: persistence / late-window prevalence vs.\ $\beta$). This separates \textbf{RQ2} threshold shifts from \textbf{RQ3} variability in endemic levels among persisting runs. Values require a fresh export of experiments~\texttt{05}--\texttt{07} after the NetLogo fix that preserves \texttt{bs-collect-metrics?} through \texttt{setup}.
...
Experiment \texttt{08-dynamics-trajectory-ER} records per-tick microscopic prevalence (grp-SIS) and the homogeneous mean-field SIS prevalence updated in the same model. Figure~\ref{fig:traj-bands} averages microscopic trajectories over replicates at three $\beta$ values near $\beta_{\mathrm{pred}}$ (low / near / high on the sweep grid), with $95\%$ bands, addressing \textbf{RQ3} (transients and run-to-run variability); the dotted mean-field curve (middle $\beta$, one replicate) checks that the in-model ODE tracks the micro state when extinction does not intervene early. Figure~\ref{fig:traj} shows one illustrative replicate at the same experiment settings.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig_er_prevalence_trajectory_bands.png}
  \caption{Experiment \texttt{08}: mean microscopic prevalence $\pm$ $95\%$ band over replicates ($24$ per cell), three recovery laws (columns) and $\beta$ levels (line styles). Regenerate after \texttt{run\_pipeline.py --experiment 08-dynamics-trajectory-ER}.}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Purpose}: export per-tick prevalence for the microscopic grp-SIS simulation and the in-model homogeneous mean-field SIS curve (Figure~\ref{fig:traj-bands}, \textbf{RQ3}).
  \item \textbf{Repetitions}: 24 per $(\beta,\text{recovery law})$ cell.
  \item \textbf{RunMetricsEveryStep}: \texttt{true} (per-tick table export; large CSV).
