...
Usage (from project root):
  ...
  py -3 scripts/write_report_macros.py --dest oplevering/report/generated_quantities.tex --student-id
"""
...
# Public GitHub repository (fallback when ``git remote`` is unset).
DEFAULT_GITHUB_REPO = "https://github.com/jruijgrok141/grp-sis-virus-simulation"
STUDENT_ID = "852796035"
...
def _report_date_latex(when: datetime | None = None) -> str:
    """Title-page date, e.g. ``5 July 2026`` (day without leading zero)."""
...
def _git_repo_url() -> str:
    """HTTPS repository URL for data-availability line (env override: REPORT_GITHUB_URL)."""
    import os

    env = os.environ.get("REPORT_GITHUB_URL", "").strip()
    if env:
        return env.rstrip("/").removesuffix(".git")
    try:
        r = subprocess.run(
...
def write_generated_quantities(dest: Path, *, include_student_id: bool = False) -> None:
    L = _load_lambda()
    lm = L["lambda_max"]
    ...
    report_date = _report_date_latex(build_time)
    git_hash = _git_hash()
    git_url = _git_repo_url()
    repo_phrase = f"; repository \\url{{{git_url}}}" if git_url else ""
    student_suffix = f" ({STUDENT_ID})" if include_student_id else ""

    lines = [
        "% -*- latex -*-",
        f"% Generated by scripts/write_report_macros.py on {build_time.isoformat(timespec='seconds')} — do not edit by hand.",
        f"\\newcommand{{\\RepReportDate}}{{{report_date}}}",
        f"\\newcommand{{\\RepStudentIdSuffix}}{{{student_suffix}}}",
        f"\\newcommand{{\\RepGitHash}}{{{git_hash}}}",
        f"\\newcommand{{\\RepRepoUrlPhrase}}{{{repo_phrase}}}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Write report/generated_quantities.tex from pipeline CSVs.")
    parser.add_argument(
        "--dest",
        type=Path,
        default=DEST,
        help="Output .tex path (default: report/generated_quantities.tex)",
    )
    parser.add_argument(
        "--student-id",
        action="store_true",
        help="Include OU student number on the title page (oplevering only).",
    )
    args = parser.parse_args()
    write_generated_quantities(args.dest.resolve(), include_student_id=args.student_id)
